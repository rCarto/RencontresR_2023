...
mf_credits(maptiles::get_credit("OpenStreetMap"), bg = "white")
dev.off()
...
* `ceramic`
* `ggmap` (`ggplot2` focused)
* `ggspatial` (`ggplot2` focused, based on `rosm`)
* `mapboxapi` (mapbox)
* `mapsapi` (google, based on `RgoogleMaps`)
* `OpenStreetMap` (requires Java)
* `RgoogleMaps` (google)
* `rosm`
* …
...
## Statistiques spatiales {.nostretch}
...
Nous pouvons aussi transformer le raster des densités en polygones de plages de densités. 
```{r}
#| eval: true
#| cache: false
#| code-fold: true
...
# Limites des classes
maxval <- max(values(r))
bks <-  unique(c(seq(0, floor(maxval), 1), maxval))
library(mapiso)
...
## Acquisition de données pour un fond de carte vectorielle {.scrollable}

Nous pouvons aussi utiliser un fond de carte vectoriel.  
Ici nous allons télécharger et préparer un fonde carte constitué : 
...
mf_export(center, "img/avignon.png", width = 768)
...
![](img/avignon.png){width="5in"}

Nous pouvons maintenant superposer les densités sur ce fond de carte 
```{r}
#| eval: true
#| results: false
#| code-fold: true
#| code-summary: "Carte finale"
iso_dens <- st_transform(iso_dens, st_crs(center))
library(sf)
library(mapsf)
mf_theme("candy", fg = "#393939")
mf_export(center, "img/avignon_dens3.png", width = 768)
mf_map(center, col = "#f2efe9", border = NA, add = TRUE)
mf_map(river, col = "#aad3df", border = "#aad3df", lwd = .5, add = TRUE)
mf_map(esp_vert, col = "#c8facc", border = "#c8facc", lwd = .5, add = TRUE)
mf_map(rail, col = "grey50", lty = 2, lwd = .2, add = TRUE)
...
![](img/avignon_dens3.png){width="5in"}
...
routes_hotel <- vector("list", nrow(hotel))
for (i in 1:length(routes_hotel)){
...
mf_export(center, "img/avignon_conf.png", width = 500)
mf_map(center, col = "#f2efe9", border = NA, add = TRUE)
mf_map(river, col = "#aad3df", border = "#aad3df", lwd = .5, add = TRUE)
mf_map(esp_vert, col = "#c8facc", border = "#c8facc", lwd = .5, add = TRUE)
mf_map(rail, col = "grey50", lty = 2, lwd = .2, add = TRUE)
mf_map(routes, col = "white", border = "white", add = TRUE)
mf_map(batiments, col = "#d9d0c9", border = "#c6bab1", lwd = 1, add = TRUE)
mf_map(routes_hotel_ag, "w", "grad",
       add = TRUE, breaks = c(1,5,10,20,33),
       lwd = c(2,5,9,13), leg_pos = "topleft", col= "#940000",
...
![](img/avignon_conf.png)

## Ressources

[Geocomputation with R](https://r.geocompx.org/) - @lovelace2019  
[Geocomputation with R](https://r.geocompx.org/fr/) - Première partie en français 
