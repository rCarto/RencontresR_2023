...
`tmap` dispose d'un mode interactif (qui utilise le package `leaflet`)
...
<iframe width="800" height="500" src="https://rcarto.shinyapps.io/cityguess/"></iframe>
...
library(sf)
library(spatstat)
resto <- st_transform(resto, "EPSG:2154")
p <- as.ppp(X = st_coordinates(resto), 
            W = as.owin(st_bbox(resto)))
ds <- density(x = p, sigma = 50, eps = 10)
plot(ds)
...
On peut amélioré la représentation des résultats en téléchargeant les tuiles OSM de la région et en transformant l'image produite en polygones isoplèthes.
...
maxval <- max(values(r))
bks <-  unique(c(seq(0, floor(maxval), 2), maxval))
library(mapiso)
iso_dens <- mapiso::mapiso(r, breaks = bks)
library(maptiles)
tiles <- get_tiles(iso_dens, zoom = 15, crop = T, cachedir = "cache")
library(mapsf)
mf_theme(mar = c(0,0,0,0))
mf_export(tiles, filename = "img/avignon_dens.png", 
          width = ncol(tiles), height = nrow(tiles))
mf_raster(tiles, add = TRUE)
mf_map(iso_dens[-1, ], "isomin", "choro",
